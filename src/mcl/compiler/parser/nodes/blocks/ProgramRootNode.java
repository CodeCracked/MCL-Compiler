...
public class ProgramRootNode extends AbstractNode
{
    ...
    public MCLError transpile(MCLTranspiler transpiler, Path target)
    {
        ...
        error = transpiler.appendToFile(packMeta, file ->
        {
            ...
            file.println("        \"description\": \"Datapack generated by MCL.\"");
            file.println("    }");
            file.println('}');
        });
        if (error != null) return error;
        ...
        for (AbstractNode namespace : namespaces)
        {
            NamespaceDefinitionNode namespaceDefinition = (NamespaceDefinitionNode) namespace;
            Path namespaceFolder = dataFolder.resolve((String)namespaceDefinition.identifier.value());
            namespaceFolder.toFile().mkdirs();

            error = namespace.transpile(transpiler, namespaceFolder);
            if (error != null) return error;
        }
